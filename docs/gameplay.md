...
- The combat HUD is designed mobile portrait first. Stage, HP, collected gems, and pause/start must fit in one thin centered top status panel without wrapping. The panel should sit below phone notch/punch-hole safe areas. The panel should use balanced columns so the Stage text does not drift too far left and the pause/start control remains inside the frame. Each status item should read horizontally with the label or icon on the left and the number on the right. HUD numbers should use a slightly italic fantasy-style treatment and sit on the visual centerline of the frame, not the mathematical box center. HP is shown as `current / max`, and gem count uses the XP gem icon.
